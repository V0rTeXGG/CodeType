<template>
  <div class="main-wrapper" @click="handleClickOutside">
    <Transition name="fade" mode="out-in">
            ...
            <div class="main-info__quantity-block">
              ...
              <button @click="nextTask" class="main-info__restart">
                <svg
                  ...
                  <path class="restart-img" :class="{'blue': this.$store.state.selectLang === 'C++'}"
                        d="M14.2502 2.50012C17.1399 2.50017 19.9365 3.52177 22.1458 5.38437C24.3551 7.24696 25.8349 9.83063 26.3235 12.6787C26.8121 15.5268 26.2782 18.4559 24.816 20.9484C23.3538 23.4409 21.0576 25.3362 18.3332 26.2994C15.6087 27.2627 12.6315 27.2317 9.92765 26.2121C7.22381 25.1925 4.96747 23.2499 3.55741 20.7275C2.14735 18.2052 1.67437 15.2656 2.22206 12.4283C2.76975 9.59098 4.30285 7.03862 6.55039 5.22233"
                        stroke="#FEB81C" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/>
                  <path class="restart-img" :class="{'blue': this.$store.state.selectLang === 'C++'}"
                        d="M2.00012 4.54187H7.44454V9.98629" stroke="#FEB81C" stroke-width="4" stroke-linecap="round"
                        stroke-linejoin="round"/>
                </svg>
              </button>
            </div>
            <div class="main-info__timer" :class="{'blue': this.$store.state.selectLang === 'C++'}">
              <span class="main-info__timer__text"
                    :class="{'blue': this.$store.state.selectLang === 'C++'}">{{ timeTask }}</span>
            </div>
            <div class="main-info__right">
              <div
                ...
                <button
                    ...
                    @click="this.isSelectedActive = !this.isSelectedActive"
                >
                  <img :src="this.selectLang.icon" alt="Selected Language Icon">
                  <svg class="main-info__select__arrow" xmlns="http://www.w3.org/2000/svg" width="24" height="25"
                       viewBox="0 0 24 25" fill="none">
                    <path :class="{'blue': this.$store.state.selectLang === 'C++'}" d="M7 9.5L12 14.5L17 9.5"
                          stroke="#FEB81C" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                  </svg>
                </button>
                <div class="main-info__select__list"
                     :class="{active: isSelectedActive, 'blue': this.$store.state.selectLang === 'C++'}">
                  <p class="main-info__select-item" :class="{'blue': this.$store.state.selectLang === 'C++'}"
                     v-for="lang of languages" :key='lang.lang' @click="selectedLang(lang.lang)">{{ lang.lang }}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <template v-else key="statisticsTemplate">
            ...
            <button @click="nextTask" class="statistics-bottom__button">
              <svg
                ...
                <path
                    ...
                    d="M5.41666 2.1959L22.9427 11.5848C25.1228 12.7528 25.0326 15.9088 22.7894 16.9503L5.26333 25.0874C3.27489 26.0106 1 24.5587 1 22.3664V4.84034C1 2.57313 3.41817 1.12528 5.41666 2.1959Z"
                    fill="#FEB81C" stroke="#FEB81C" stroke-width="2" stroke-linecap="round"/>
              </svg>
            </button>
...
export default {
  ...
  methods: {
    ...
    handleClickOutside(event) {
      const selectControl = this.$el.querySelector('.main-info__select__control');
      const selectBlock = this.$el.querySelector('.main-info__select');

      if (selectControl && selectBlock && !selectBlock.contains(event.target)) {
      ...
      if (!this.forbiddenKeys.includes(event.keyCode)) {
        ...
        if (event.key === 'Backspace') {
          if (this.activeChar > 0) {
            ...
            if (this.incorrectIndexes.includes(this.activeChar)) {
              this.incorrectIndexes = this.incorrectIndexes.filter(item => item !== this.activeChar);
            }
          }
        } else {
  ...
  computed: {
    ...
    accuracy() {
      return Math.round((100 - (this.errorInput / this.inputChar) * 100))
    },
    charPerMinute() {
